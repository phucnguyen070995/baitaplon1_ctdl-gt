...
int ProcessData::insert(const string *sp, const int n){
	...
	if (FindingExchange != nullptr)
	{
		...
		else if (timeIns == timedataCandle) {
			delete newCandle;
			flag = 1;
		}
		...
		if (n==5){
			...
			while (FindingCandle != nullptr){
				...
				if (timeA <= timedataCandle && timedataCandle <= timeB) {
					if ((max(CP, OP) - min(OP, CP) - 0.00005 > EP) && (max(CP, OP) == HP) && (min(CP, OP) == LP)) {count++;}
				}
	...
	if (FindingExchange != nullptr){
		...
		if (n==5){
			...
			while (FindingCandle != nullptr){
				...
				float CP = dataCandle.getCP();
				if (timeA <= timedataCandle && timedataCandle <= timeB) {
					if (((max(CP, OP) - min(CP, OP) <= 0.00005) || (max(CP, OP) - min(CP, OP) - 0.00005 < EP)) && (HP - max(CP, OP) - 0.00005 > EP) && (min(CP, OP) - LP - 0.00005 > EP)) {count++;}
				}
				if (timedataCandle > timeB) {
					break;
				}
