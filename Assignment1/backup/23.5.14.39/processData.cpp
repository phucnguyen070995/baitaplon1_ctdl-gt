...
//Ham object Exchange
Exchange::Exchange(string BC, string QC)
{
	this->BC = BC;
	this->QC = QC;
	this->numCandle = 1;
...
//Ham object Candle
void Candle::SetCandle(long TIME, float OP, float HP, float LP, float CP)
{
	this->TIME = TIME;
	this->OP = OP;
	this->HP = HP;
...
bool ProcessData::isCandle(string time, string OP, string HP, string LP, string CP, long &time1, float &OP1, float &HP1, float &LP1, float &CP1){
...
int ProcessData::insert(const string *sp, const int n){
	...
	float LP = 0;
	float CP = 0;
	if (!isCandle(sp[3], sp[4], sp[5], sp[6], sp[7], timeIns, OP, HP, LP, CP)) {return -1;}
	Link<Exchange> *FindingExchange = FindExchange(sp[1], sp[2]);
	//Tao Link<Candle>
	Candle obCandle;
	obCandle.SetCandle(timeIns, OP, HP, LP, CP);
